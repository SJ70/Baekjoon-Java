...
//            printMap("put");
            for(int color=GREEN; color<=BLUE; color++){
                while(true){
                    isErasedLine = false;
                    eraseLines(color);
                    if(!isErasedLine) break;
                    fallFloatedBlocks(0,R-1,color);
                }
...
//        printMap("erase");
    ...
    public static void fallFloatedBlocks(int rFrom, int rUntil, int color){
        for(int r=rUntil; r>=rFrom; r--){
            for(int c=0; c<C; c++){
                if(Map[color][r][c]>0){
                    fallBlock(color,r,c);
//                    printMap("fallblock:"+r+","+c);
                }
            }
        }
    }
